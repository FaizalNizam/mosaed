...
const Welcomescreen = (props) => {
  ...
  return (
    <View style={{ height: height * 0.9999,backgroundColor:'white' }}>

      <LinearGradient
        colors={['#2ECBAA','#387FDA']}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 0 }}
        style={{ height: height * 0.341, alignItems: 'center', justifyContent: 'center' }}>
        <Image source={logo} />
      </LinearGradient>
      ...
      <View style={{ marginHorizontal: width * 0.074 }}>
        ...
        <Text style={{ fontSize: 16, color: '#363636', marginTop: height * 0.024 }}>{props.sub}</Text>
        <TextInput
          style={{ height: height * 0.065, width: width * 0.86, borderWidth: 1, borderColor: '#3D989F', borderRadius: width * 0.021, opacity: 0.5, marginTop: height * 0.045, paddingLeft: width * 0.025}}
          placeholder={props.placeholder}
          placeholderTextColor={'#3D989F'}
        />
        <Buttons title={'send code'} press={props.name === 'homescreen' ? () => setVisible(!visible) : handleNext} margintop={height * 0.048} width={width * 0.86} />
      </View>
